...
public class FileManager {
    ...
    public Map<String, AbstractProduct> loadInventory() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(PRODUCT_FILE))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                switch (type) {
                    case "FOOD":
                        inventory.put(slot, new FoodItemsProducts(id, slot, name, price, stock, Integer.parseInt(extra)));
                        break;
                    case "BEVERAGES":
                        inventory.put(slot, new BeveragesProducts(id, slot, name, price, stock, Boolean.parseBoolean(extra)));
                        break;
                    case "TECHNOLOGICAL":
                        inventory.put(slot, new TechnologicalProducts(id, slot, name, price, stock, extra));
                        break;
                    case "PET":
                        inventory.put(slot, new PetProducts(id, slot, name, price, stock, extra));
                        break;
                }
            }
    ...
    public void saveInventory(Map<String, AbstractProduct> inventory) {
        try (PrintWriter pw = new PrintWriter(new FileWriter(PRODUCT_FILE))) {
            for (AbstractProduct p : inventory.values()) {
                ...
                if (p instanceof FoodItemsProducts) {
                    type = "FOODS";
                    extra = String.valueOf(((FoodItemsProducts) p).getCalorieCount());
                } else if (p instanceof BeveragesProducts) {
                    type = "BEVERAGES";
                    extra = String.valueOf(((BeveragesProducts) p).isCooled());
                } else if (p instanceof PetProducts) {
                    type = "PET";
                    extra = ((PetProducts) p).getPetType();
                } else if (p instanceof TechnologicalProducts) {
                    type = "TECHNOLOGİCAL";
                    extra = ((TechnologicalProducts) p).getDeviceType();
                }
    ...
    // YÖNETİCİ sifre kontrol fnksiyonu
